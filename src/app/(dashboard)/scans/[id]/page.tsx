...
interface Finding {
  ...
  asset: { id: string; name: string; ipAddress: string | null } | null;
  createdAt: string;
}
...
export default function ScanDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleExecute() {
    setExecuting(true);
    try {
      // Execute batches until completed
      let status = "running";
                          ...
                          <div className="flex gap-2 pt-2">
                            {finding.status === "open" && (
                              <>
                                <Button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleUpdateFindingStatus(finding.id, "acknowledged");
                                  }}
                                >
                                  Acknowledge
                                </Button>
                                <Button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleUpdateFindingStatus(finding.id, "resolved");
                                  }}
                                >
                                  Resolve
                                </Button>
                                <Button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleUpdateFindingStatus(finding.id, "false_positive");
                                  }}
                                >
                                  False Positive
                                </Button>
                            ...
                            {finding.status !== "open" && (
                              <Button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleUpdateFindingStatus(finding.id, "open");
                                }}
                              >
                                Reopen
                              </Button>
